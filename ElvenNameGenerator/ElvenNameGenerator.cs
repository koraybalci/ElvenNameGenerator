...
namespace ElvenNameGenerator
{
	public class ElvenNameGenerator
	{
		...
		public IEnumerable<string> GenerateFullName()
		{
			while (true)
			{
				yield return $"{GenerateName()} {GenerateName()}";
			}
		}

		public string GenerateName()
		{
			var optional = _random.Next(2) == 0 ? _suffixes.PickRandomItem(_random) : string.Empty;
			return $"{_prefixes.PickRandomItem(_random)}{_suffixes.PickRandomItem(_random)}{optional}";
		}
	}
}
